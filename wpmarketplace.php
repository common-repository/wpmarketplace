<?php 
    ...
    Description: WordPress Marketplace is a free full-featured Shopping Cart / eCommerce Plugin with everything you need on an easy UI, to build eShop or marketplace.
    Author: Shaon
    Version: 2.4.1
...
//                echo "</pre>";
    ...
    function wpmp_extends(){
        require_once WPMP_BASE_DIR .'libs/extends.php';
    }
    
    function wpmp_extension_styles(){
        wp_enqueue_style('wp-marketplace', plugins_url() . '/wpmarketplace/bootstrap/css/bootstrap.css');
        wp_enqueue_style('wp-extends-css', plugins_url() . '/wpmarketplace/css/extends_page.css');
    }
    
    function wpmp_extension_scripts(){
    ...
    function wpmp_menu(){     
        ...
        $ext_hook = add_submenu_page( 'edit.php?post_type=wpmarketplace', __('Extends &lsaquo; Marketplace',"wpmarketplace"), __('Extends',"wpmarketplace"), 'level_2', 'extends', 'wpmp_extends');    
        add_action( 'admin_print_scripts-' . $ext_hook, 'wpmp_extension_scripts' );
        add_action('admin_print_styles-'.$ext_hook,'wpmp_extension_styles');
        $hook = add_submenu_page( null, __('Product Report',"wpmarketplace"), "", 'level_2', 'product-report', create_function('', 'require_once WPMP_BASE_DIR . "libs/product_report.php";'));    
    ...
    function wpmp_download(){    
        $get_file = isset($_GET['wpmpfile']) ? (int) $_GET['wpmpfile'] : 0 ;
        if(!$get_file) return;
        ...
        $meta = get_post_meta($get_file,"wpmp_list_opts",true);
        ...
        if(@in_array($get_file,$items) && $_GET['oid'] != '' && is_user_logged_in() && $current_user->ID == $odata->uid && $odata->payment_status=='Completed'){   
            ...
            @extract(get_post_meta($get_file,"wpmp_list_opts",true));
            include("libs/process.php");
        }
    }
    ...
    function wpmp_place_order(){
        if(isset($_REQUEST['wpmpaction']) && $_REQUEST['wpmpaction']=='placeorder'){
            ...
            do_action('wpmp_before_placing_order',$_SESSION['orderid']);
            if($order_total>0){
                $payment = new Payment();
    ...
    //function for buyer shortcode tabs
    
    function wpmp_frontend_buyer(){
        //$cap = 'wpmarketplace_user'; 
        //if(!current_user_can($cap)):
            include("tpls/wpmp_frontend_buyer.php");
        //endif;
    }
    
    //function for saler shortcode tabs
    function wpmp_frontend_saler(){
        $cap = 'wpmarketplace_user'; 
        if(current_user_can($cap)):
            include("tpls/wpmp_frontend_saler.php");
        endif;
    }
    ...
    function wpmp_enqueue_scripts(){
        global $wpmp_plugin,$current_screen;
        ...
        $settings = get_option('_wpmp_settings'); 
        if((is_admin() && $current_screen->post_type=="wpmarketplace")||(!is_admin() && !isset($settings['disable_fron_end_css']))){
            wp_enqueue_style('wp-marketplace-bt-css', plugins_url() . '/wpmarketplace/bootstrap/css/bootstrap.css');
            wp_enqueue_script('wp-marketplace-bt-js', plugins_url() . '/wpmarketplace/bootstrap/js/bootstrap.min.js',array('jquery'));
        }
